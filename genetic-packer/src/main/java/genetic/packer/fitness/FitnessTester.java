...
public class FitnessTester implements Function<Individual, Double> {
    ...
    public Double apply(final Individual individual) {
        final List<BoxScoreProperties> boxScorePropertiesList = individual.getCells().stream().map(BoxScoreProperties::new).collect(Collectors.toList());

        final List<Bounds> allBoxesBounds = individual.getCells().stream().map(Node::getBoundsInParent).collect(Collectors.toList());
